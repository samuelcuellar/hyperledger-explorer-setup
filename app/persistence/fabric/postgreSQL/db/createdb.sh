...
#support for OS
case $OSTYPE in
darwin*) psql postgres -v dbname=$DATABASE -v user=$USER -v passwd=$PASSWD -f /opt/explorerpg.sql ;
psql postgres -v dbname=$DATABASE -v user=$USER -v passwd=$PASSWD -f /opt/updatepg.sql ;;
linux*)
...
${PSQL} -v dbname=$DATABASE -v user=$USER -v passwd=$PASSWD -f /opt/explorerpg.sql ;
${PSQL} -v dbname=$DATABASE -v user=$USER -v passwd=$PASSWD -f /opt/updatepg.sql ;;
esac
